defmodule B2.AccountsTest do
  use B2.DataCase

  alias B2.Accounts
  import B2.AccountsFixtures
  alias B2.Accounts.{User, UserToken}
  ...
  describe "register_user/1" do
    test "requires email and password to be set" do
      ...
      assert %{
               password: ["Please provide a value"],
    ...
    test "validates maximum values for email and password for security" do
      ...
      assert "Password should be a min of 6 characters and max of 80 characters" in errors_on(
               changeset
             ).password
    end
  ...
  describe "change_user_registration/2" do
    test "returns a changeset" do
      ...
      assert changeset.required == [:password, :email, :firstname, :lastname, :salutation]
    end
  end
  ...
  describe "apply_user_email/3" do
    ...
    test "requires email to change", %{user: user} do
      ...
      assert %{email: ["Your email did not change"]} = errors_on(changeset)
    end
    ...
    test "validates current password", %{user: user} do
      ...
      assert %{current_password: ["Your password is not valid"]} = errors_on(changeset)
    end
  ...
  describe "update_user_password/3" do
    ...
    test "validates password", %{user: user} do
      ...
      # assert %{
      #          password: ["Password should be at least 6 character(s)"],
      #          password_confirmation: ["Your passwords do not match"]
      #        } = errors_on(changeset)
    end
  ...
  describe "confirm_user/2" do
    setup do
      ...
      %{unconfirmed_user: unconfirmed_user, token: token}
    end

    test "confirms the email with a valid token", %{unconfirmed_user: unconfirmed_user, token: token} do
    ...
    test "does not confirm email if token expired", %{unconfirmed_user: unconfirmed_user, token: token} do
      {1, nil} = Repo.update_all(UserToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
